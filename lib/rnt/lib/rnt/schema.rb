...
module Rnt
  ...
  class Schema # rubocop:disable Metrics/ClassLength
    ...
    # Use current folder
    SCHEMAS_FOLDER = File.expand_path("../../lib/rnt/schemas/", __dir__).freeze
    ...
    def self.schema_version(xml_string)
      xml_string[/<rnt\s+[^>]*?version=["']([^"']+)["']/, 1] if xml_string
    end
    ...
    def initialize(rnt_version: VERSION, schema_version: SCHEMA_VERSION, xsd_path: nil) # rubocop:disable Metrics/CyclomaticComplexity
      ...
      @xsd_path = xsd_path || File.join(SCHEMAS_FOLDER, "mdd_v#{schema_version}.xsd").to_s
      unless File.exist?(@xsd_path) && schema_version.end_with?(".0")
        @xsd_path = File.join(SCHEMAS_FOLDER, "mdd_v0.#{schema_version}.xsd").to_s
      end
    ...
    def element_names_with_sources # rubocop:disable Metrics/AbcSize
      xsd.xpath("//xs:element").each_with_object({}) do |element, elt_name_with_sources|
        ...
        elt_name_with_sources[element["name"]] = nil unless appinfo&.any?
    ...
    def extract_elt_attributes(element) # rubocop:disable Metrics/MethodLength
      has_enum = element.at_xpath(".//xs:simpleType/xs:restriction/xs:enumeration")&.any?
      type = if has_enum
               "enum"
             else
    ...
    def openapi_path
      File.join(SCHEMAS_FOLDER, "mdd_v#{schema_version}.json").to_s
    end
    ...
    def valid?(xml_path)
      ...
      if errors.empty?
        puts "✅ XML is valid against schema"
        true
      else
        puts "❌ XML failed validation:"
        errors.each { puts " - #{it.message} (line #{it.line})" }
        false
      end
    end
