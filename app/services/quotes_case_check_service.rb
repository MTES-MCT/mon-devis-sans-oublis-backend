...
class QuotesCaseCheckService
  ...
  def reset_quotes_check! # rubocop:disable Metrics/MethodLength
    quotes_case.quote_checks.each do |quote_check|
      validation_error_detail_id_to_remove = (quote_check.validation_error_details || []).filter_map do
        it.fetch("id") if it.fetch("category") == "case_incoherence"
      end

      quote_check.update!(
        validation_controls_count: quote_check.validation_controls_count &&
        (quote_check.validation_controls_count - quote_check.validation_controls_count),
        validation_control_codes: quote_check.validation_control_codes &&
